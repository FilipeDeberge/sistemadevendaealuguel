...
namespace Sistema
{
    public partial class frm_categorias : Form
    {
        ...
        private void btn_excluir_Click(object sender, EventArgs e)
        {
            ...
            {
                if (this.CategoriaPossuiVestuario(this.categoriaAtual))
                    MessageBox.Show("Você não pode excluir essa categoria, porque existem vestuários nela.");
                else
                {
        ...
        private bool CategoriaPossuiVestuario(Categoria categoria)
        {
            var vestuarios = DataContextFactory.DataContext.Vestuario.Where(x => x.CodigoCategoria == categoria.Codigo);
            if (vestuarios.Count() > 0)
                return true;
            else
                return false;
        }
